...
type TenantLoginValues = z.infer<typeof loginSchema>

interface TenantLoginPageProps {
  params: Promise<{ eventId: string }>
}

function getTenantRedirectPath(invitationCode: string, eventId: string) {
...
export default function TenantLoginPage({ params }: TenantLoginPageProps) {
  ...
  const handleSubmit = async (values: TenantLoginValues) => {
    setLoading(true)
    setError('')
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Data tenant tidak valid.')
      }

      router.push(getTenantRedirectPath(data.invitation_code, eventId))
    } catch (err: unknown) {
                      ...
                      <div className="relative mx-auto h-40 w-full max-w-[280px] overflow-hidden md:h-56 md:max-w-[380px]">
                        {event?.logo_url ? (
                          ...
                          <div className="bg-halal-secondary flex h-full w-full items-center justify-center">
                            <Store className="text-halal-primary/40 h-20 w-20" />
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col justify-center bg-black/20 p-8 lg:p-10">
                    <div className="mb-8">
                      ...
                      <CardDescription className="text-slate-400">
                        Silahkan masukkan nomor WhatsApp tenant yang telah didaftarkan untuk mengakses data booth dan tiket.
                      </CardDescription>
                    </div>
                    ...
                    <Form {...form}>
                      ...
                      >
                        ...
                        <div className="text-center">
                          <Button
                            variant="link"
                            className="text-halal-primary text-xs"
                            onClick={() => router.push(`/register/${eventId}/tenant`)}
                          >
                            Belum mendaftar sebagai tenant? Daftar di sini
                          </Button>
                        </div>
                      </form>
                    </Form>
